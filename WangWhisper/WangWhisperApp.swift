...
class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        if let appIconPath = Bundle.main.path(forResource: "AppIcon", ofType: "icns"),
           let iconImage = NSImage(contentsOfFile: appIconPath) {
        ...
        WangWhisperApp.startTranscriptionQueue()
    ...
    private func setupStatusBarItem() {
        ...
        if let button = statusItem?.button {
            if let iconImage = NSApp.applicationIconImage.copy() as? NSImage {
                iconImage.size = NSSize(width: 18, height: 18)
                iconImage.isTemplate = false
                button.image = iconImage
            } else {
                button.image = NSImage(systemSymbolName: "waveform", accessibilityDescription: "youmi-uu")
            }
    ...
    private func updateStatusBarMenu() {
        ...
        menu.addItem(NSMenuItem(title: "youmi-uu", action: #selector(openApp), keyEquivalent: "o"))
